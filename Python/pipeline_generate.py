...
更新说明：
	...
	* 添加了Depend参数，可以更加自由的设置任务的前置任务，可以更加灵活控制任务进程。同时保留了层级结构，便于理解。但是要求name是唯一的。
	2016年12月14日 v6.2
	* 修复了-c参数时候，由于sample 未sorted，导致每次脚本不一致的情况
	2016年12月26日 
	* 修复了-a参数时，job count使用重复的情况，导致流程中断。修改了本脚本，lib/parseConfig.py和src/qsub_sge.py
	2017-01-17：
	* 修复了每次生成脚本不一致的问题，在join(set()) 改成了 join(sorted(set()))
	2018-02-22
	* v8 将qsub_sge.py修改成qsub_sge , 封装了环境变量，同时drmaa来替代底层的通信
To Be Done:
...
import os
import re
bindir = os.path.abspath(os.path.dirname(os.path.abspath(os.path.dirname(__file__))))
sys.path.append('{0}/lib'.format(bindir))
import parseConfig

__author__='LT'
__mail__= 'LT@tech.com'
...
说明：
	...
	Q:如何监控项目运行状态？
	A:运行show_process.py会显示项目的状态。项目运行状态分为running, break, plan, end 四种。其中running表示正在运行，break表示中断，plan表示准备运行，end表示运行完成,hold表示磁盘不够，任务挂起。
